...
export const DatasetDownloadCard: React.FC<DatasetDownloadCardProps> = ({
  ...
  return (
    <div
      id="dataset-card"
      className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden"
    >
      ...
      <div className="p-6 sm:p-8">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-start gap-4">
            <div className="w-14 h-14 rounded-2xl bg-[#107C41]/10 dark:bg-emerald-950/40 text-[#107C41] dark:text-emerald-400 border border-[#107C41]/20 dark:border-emerald-800/50 flex items-center justify-center shrink-0 shadow-2xs">
              <FileSpreadsheet className="w-7 h-7 text-[#107C41] dark:text-emerald-400" />
            </div>
            ...
            <div>
              ...
              <p className="text-sm text-slate-600 dark:text-slate-300 mt-1.5 max-w-2xl leading-relaxed">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap lg:flex-nowrap shrink-0">
            <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2 text-center min-w-[80px]">
            ...
            <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2 text-center min-w-[76px]">
              <div className="text-[10px] uppercase font-semibold text-slate-400 dark:text-slate-500 tracking-wider">
                Format
              </div>
              <div className="text-base font-bold text-[#107C41] dark:text-emerald-400 font-mono">.XLSX</div>
            </div>
          </div>
        </div>
        ...
        <div className="py-5">
          <div className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2.5 flex items-center justify-between flex-wrap gap-2">
            ...
            <span className="text-slate-400 dark:text-slate-500 normal-case font-normal text-[11px]">
              Top Region: <strong className="text-slate-700 dark:text-slate-200 font-semibold">{stats.topRegion}</strong> (${(stats.topRegionRevenue / 1000000).toFixed(1)}M)
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {[
              ...
              <span
                key={col.name}
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs bg-slate-100/90 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 font-mono"
                title={`${col.name}: ${col.type}`}
              >
                <span className="font-semibold text-slate-800 dark:text-slate-200">{col.name}</span>
                <span className="text-[10px] text-slate-400 dark:text-slate-500 font-sans">({col.type})</span>
              </span>
            ))}
          </div>
        </div>
        ...
        <div className="pt-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 border-t border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 px-4 py-3.5 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-200 font-semibold text-xs sm:text-sm border border-slate-300 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-600 shadow-2xs transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 px-4 py-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 hover:bg-amber-100 dark:hover:bg-amber-900/60 text-amber-900 dark:text-amber-200 font-bold text-xs sm:text-sm border border-amber-300/80 dark:border-amber-800 shadow-2xs transition-all cursor-pointer disabled:opacity-50"
              title="Generate a completely new randomized scenario and non-repeating dataset"
            >
              <Dices className={`w-4 h-4 text-amber-700 dark:text-amber-400 ${isRandomizing ? 'animate-spin' : ''}`} />
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors cursor-pointer"
      ...
      {/* Micro Info Bar */}
      <div className="px-6 sm:px-8 py-3 bg-slate-50 dark:bg-slate-850/60 border-t border-slate-200/80 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 flex-wrap gap-2">
        ...
        <span className="text-[11px] text-slate-400 dark:text-slate-500">
          Current Scenario: <strong className="text-slate-600 dark:text-slate-300">{scenario.themeTitle}</strong>
        </span>
      </div>
    </div>
  );
