    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 sm:p-6 mb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-5 pb-4 border-b border-slate-100">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="text-xs font-medium text-slate-500">Step-by-Step Procedure</span>
          </div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight">{tutorial.title}</h2>
          <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">{tutorial.subtitle}</p>
        </div>

        {/* Objective Pill */}
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 sm:max-w-md">
          <div className="text-[11px] uppercase tracking-wider font-bold text-emerald-800 mb-1 flex items-center gap-1.5">
        ...
        {tutorial.steps.map((step) => {
          ...
          return (
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {step.excelAction && !isExpanded && (
                      <p className="text-xs text-slate-500 truncate max-w-xl mt-0.5 font-mono">
                        {step.excelAction}
                      </p>
                    )}
                  </div>
              ...
              {isExpanded && (
                <div className="px-4 pb-4 pt-1 text-xs text-slate-700 space-y-3.5 border-t border-slate-100 mt-1">
                  <p className="leading-relaxed text-slate-600 text-sm">{step.description}</p>

                  {/* Excel Action Ribbon / Shortcut */}
                  {step.excelAction && (
                    <div className="bg-slate-50 border border-slate-200/90 rounded-lg p-2.5 flex items-center gap-2">
                  ...
                  {step.formula && (
                    <div className="bg-slate-900 text-slate-100 rounded-xl p-3.5 border border-slate-800 shadow-sm">
                      ...
                      <div className="font-mono text-xs text-emerald-300 bg-slate-950/80 p-2.5 rounded-lg overflow-x-auto border border-slate-800 mb-2.5 selection:bg-emerald-900">
                        {step.formula.code}
                      </div>
                        ...
                        <div className="space-y-1.5 pt-2 border-t border-slate-800">
                          ...
                          <div className="grid grid-cols-1 gap-1.5">
                            {step.formula.breakdown.map((item, idx) => (
                              <div
                                key={idx}
                                className="text-[11px] bg-slate-800/60 p-1.5 rounded flex items-start gap-2"
                              >
                  ...
                  {step.proTip && (
                    <div className="bg-amber-50/70 border border-amber-200/80 rounded-xl p-3 flex items-start gap-2.5 text-amber-900">
                      ...
                      <div className="text-xs leading-relaxed">
                        <strong className="font-semibold text-amber-950">Pro-Tip: </strong>
                        {step.proTip}
                      </div>
                    </div>
                  )}
      ...
      {tutorial.alternativeFormulas && tutorial.alternativeFormulas.length > 0 && (
        <div className="mt-6 pt-5 border-t border-slate-100">
          <h3 className="text-xs uppercase tracking-wider font-bold text-slate-500 mb-3 flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {tutorial.alternativeFormulas.map((alt, idx) => (
              <div
                key={idx}
                className="bg-slate-50 rounded-xl p-3.5 border border-slate-200/80 flex flex-col justify-between"
              >
                <div>
                  ...
                  <div className="font-mono text-xs text-slate-800 bg-white p-2 rounded border border-slate-200 mb-2 overflow-x-auto">
                    {alt.code}
                  </div>
                </div>
                <p className="text-[11px] text-slate-500 leading-normal">{alt.note}</p>
              </div>
            ))}
          </div>
        </div>
